...
# ------------------------------------------------------------------------------------------------------------
# Species name must be in a column names "Species"
# Specimen name must be in a column named "Specimen"
# ------------------------------------------------------------------------------------------------------------
...
mean_range <- mapply(function(col, cutoff) calculate_specimen_mean_range(data, col, cutoff), numcol, cutoffs, SIMPLIFY = F)
...
for (i in seq(length(numcol))) {
  ...
  writeData(wb, numcol[i], paste("Range of specimen means and number of measurements for", numcol[i], ", cutoff = ", cutoffs[i], " measurements per specimen", sep=""), startRow = nrow(stats[[i]]) + 4)
...
# Write the title
writeData(wb, "Stable Measurements", paste("Number of specimens to achieve stable means, stability within", stability), startRow = 1, startCol = 1)
