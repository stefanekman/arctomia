# +----------------------------------------------------------------------+
# | This script performs machine learning using logistic regression with |
# | L2 (Ridge) regularisation.                                           |
# |                                                                      |
# | Stefan Ekman, 5 Dec 2024                                             |
# +----------------------------------------------------------------------+
...
# Function for calculating Shannon entropies
calculate_entropy <- function(probs) {
	-rowSums(probs * log2(probs + 1e-15))
}
...
caret::confusionMatrix(as.factor(predicted_class), y_train) %>% capture.output(file="arctomia_lrl2.txt", append=T)
