...
suppressWarnings({
	model <- cv.glmnet(X_train2, y_train2, family = "multinomial", alpha = 0, type.measure="class", nfolds=100)
})

# Model performance on original training set
trainprob <- predict(model, newx = X_train, type = "response", s = "lambda.1se")
...
testprob <- predict(model, newx = X_test, type = "response", s = "lambda.1se")
