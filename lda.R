...
table <- read.table("~/R_workdir/arctomia_morph.txt", sep="\t", header=T, row.names=1)
alldata <- table
...
confmatr <- table(train$label, pred$class)
...
arcol <- rgb(200, 200, 200, max=255, alpha = 150)

png("lda.png", width=20, height=20, unit="cm", res=600)

plot(arclda, dimen=2, panel=function(x,y) points(x,y, pch=16, cex=2, col=colnum))
...
# is useful for one of two purposes: (1) to model multivariate nonnormality or nonlinear re- lationships among variables within each group, 
